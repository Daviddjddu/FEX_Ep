...
class Sequence(nn.Module):
    def __init__(self):
        ...
        # LSTM layers with input size 3 (S, I, R) and hidden size 51
        self.lstm1 = nn.LSTMCell(3, 51)  # Input is 3 (S, I, R)
        self.lstm2 = nn.LSTMCell(51, 51)  # Hidden layer size 51
        self.linear = nn.Linear(51, 3)  # Output is 3 (S, I, R)
...
if __name__ == '__main__':
    ...
    # Load SEIR data (shaped [100, 250, 3]) using NumPy
    train_data = np.load('Data/sir_training_data.npz')['data']  # Load the data from the .npz file
    test_data = np.load('Data/sir_testing_data.npz')['data']  # Load the test data
    ...
    input = train_data[:, :-1, :]  # Training input: [100, 249, 3]
    target = train_data[:, 1:, :]  # Training target: [100, 249, 3]
    
    # Use the first time step from the test data correctly, shape: [100, 3]
    test_input = test_data[:, 0, :]  # Correctly extract the first time step as [100, 3]
    
    # True test data for comparison (excluding the first time step), shape: [100, 249, 3]
    ...
    plt.plot(range(250), y_pred[5, :, 1], linewidth=5, label='Predicted I (Infectious)', color='red', linestyle = ':')
    plt.plot(range(250), y_pred[5, :, 2], linewidth=5, label='Predicted R (Recovered)', color='green')
    ...
    plt.savefig(f"Plot/Predicted_trajectory_RNN_SIR_{6}.png", dpi=300, bbox_inches="tight")
    ...
    testing_error = np.mean(mse_per_time_step)
    print(f"Testing Error (Mean MSE): {testing_error:.8f}")
    
    # Save training losses for future plotting
    np.save('Data/sir_train_losses_RNN.npy', np.array(train_losses))
    np.save('Data/mse_RNN_SIR.npy', mean_mse_curve)
    np.save("Data/modified_mse_RNN_SIR.npy", conf_interval)

    
    # # Save MSEs for each time step
    # np.save('mse_RNN_SIR.npy', mse_per_time_step)
...
#     draw(y_true[0, :, 1], y_pred[0, :, 1], 'b', 'I')
#     draw(y_true[0, :, 2], y_pred[0, :, 2], 'y', 'R')


#     plt.legend()
#     plt.savefig('sir_predict_%d.pdf' % 1)
#     plt.close()
