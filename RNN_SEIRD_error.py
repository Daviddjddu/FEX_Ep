...
class Sequence(nn.Module):
    def __init__(self):
        ...
        # LSTM layers with input size 5 (S, E, I, R, D) and hidden size 51
        self.lstm1 = nn.LSTMCell(5, 51)  # Input is 5 (S, E, I, R, D)
        self.lstm2 = nn.LSTMCell(51, 51)  # Hidden layer size 51
        self.linear = nn.Linear(51, 5)  # Output is 5 (S, E, I, R, D)
...
if __name__ == '__main__':
    ...
    # Load SEIR data (shaped [100, 250, 5]) using NumPy
    train_data = np.load('Data/seird_training_data.npz')['data']  # Load the data from the .npz file
    test_data = np.load('Data/seird_testing_data.npz')['data']  # Load the test data
    ...
    input = train_data[:, :-1, :]  # Training input: [100, 249, 5]
    target = train_data[:, 1:, :]  # Training target: [100, 249, 5]
    
    # Use the first time step from the test data correctly, shape: [100, 5]
    test_input = test_data[:, 0, :]  # Correctly extract the first time step as [100, 5]
    
    # True test data for comparison (excluding the first time step), shape: [100, 249, 5]
    ...
    # Reshape the initial input to match the format [batch_size, 1, 5]
    initial_input = test_input.unsqueeze(1).detach().numpy()  # Shape: [100, 1, 5]

    # Concatenate the initial time step to both the predicted and true sequences
    y_pred = np.concatenate((initial_input, y_pred), axis=1)  # Shape: [100, 250, 5]
    y_true = np.concatenate((initial_input, y_true), axis=1)  # Shape: [100, 250, 5]
    ...
    plt.plot(range(250), y_pred[5, :, 4], linewidth=5, label='Predicted D (Deceased)', color='black')
    ...
    plt.savefig(f"Plot/Predicted_trajectory_RNN_SEIRD_{6}.png", dpi=300, bbox_inches="tight")
    ...
    np.save('Data/seird_train_losses_RNN.npy', np.array(train_losses))
    np.save('Data/mse_RNN_SEIRD.npy', mean_mse_curve)
    np.save("Data/modified_mse_RNN_SEIRD.npy", conf_interval)
    
    # # Save MSEs for each time step
    # np.save('mse_RNN_SEIRD.npy', mse_per_time_step)

#     # Draw the results for the first batch
#     plt.figure(figsize=(30, 10))
#     plt.title('Predict future values for SEIRD model\n(Dashlines are predicted values)', fontsize=30)
...
#     draw(y_true[0, :, 4], y_pred[0, :, 4], 'o', 'D')

#     plt.legend()
#     plt.savefig('seird_predict_%d.pdf' % 1)
#     plt.close()
