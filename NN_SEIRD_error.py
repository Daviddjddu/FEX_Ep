...
# Function to prepare input-output pairs for regression (for S, E, I, R, D dimensions)
def prepare_data(trajectories, dt):
    ...
    for i in range(num_simulations):
        for j in range(num_steps - 1):
            # Append the current state (S_i, E_i, I_i, R_i, D_i) to X
            X.append(trajectories[i, j, :5])  # Taking the first five columns (S, E, I, R, D)
            # Calculate the rate of change for five dimensions and append to y
            y.append((trajectories[i, j+1, :5] - trajectories[i, j, :5]) / dt)
...
train_data = np.load('Data/seird_training_data.npz')['data']  # Updated dataset file for SEIRD model
test_data = np.load('Data/seird_testing_data.npz')['data']    # Updated dataset file for SEIRD model

# Define the time step as dt
dt = 0.2  

# Prepare data for training and testing (for S, E, I, R, D dimensions)
...
# Define the SEIRDNet class with 5 inputs (S, E, I, R, D) and 5 outputs (dS/dt, dE/dt, dI/dt, dR/dt, dD/dt)
class SEIRDNet(nn.Module):
    def __init__(self):
        super(SEIRDNet, self).__init__()
        self.fc1 = nn.Linear(5, 64)  # Input layer: 5 features (S, E, I, R, D)
        self.fc2 = nn.Linear(64, 32)  
        self.fc3 = nn.Linear(32, 5)  # Output layer: 5 features (dS/dt, dE/dt, dI/dt, dR/dt, dD/dt)
...
model = SEIRDNet().double()  # This will set all layers to float64
...
torch.save(model.state_dict(), 'Data/seird_regression_model.pt')
...
def simulate_future_values(model, X_initial, num_steps, dt):
    """Simulates future values using the model and initial values (S, E, I, R, D)."""
    X_simulated = [X_initial]

    current_state = X_initial.clone().double()  # Ensure it's in double precision
    for _ in range(num_steps):
        rate_of_change = model(current_state)  # Predict dS/dt, dE/dt, dI/dt, dR/dt, dD/dt
...
# Convert X_initial to double precision for SEIRD model (S, E, I, R, D)
X_initial = torch.tensor(test_data[:, 0, :5], dtype=torch.float64)
...
plt.plot(range(250), X_simulated[5, :, 4], linewidth=5, label='Predicted D (Deceased)', color='black')
...
plt.savefig(f"Plot/Predicted_trajectory_NN_SEIRD_{6}.png", dpi=300, bbox_inches="tight")

# Calculate the MSE for each time step (for SEIRD model, 5 dimensions)
y_true = test_data[:, 0:, :5]  # True values starting from time step 0
...
np.save('Data/mse_NN_SEIRD.npy', mean_mse_curve)
np.save("Data/modified_mse_NN_SEIRD.npy", conf_interval)
np.save('Data/seird_train_losses_NN.npy', np.array(train_losses))