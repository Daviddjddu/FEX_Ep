...
parser.add_argument('--whichDim', default=0, type=int)
...
file_path = 'Hubei_fex_normal.csv'
data = pd.read_csv(file_path)
num_rows = int(data.shape[0] * 9 / 10)
data_array = data.values[:num_rows, :]
pairs = []
for t in range(data_array.shape[0] - 1):
    pairs.append([data_array[t, :], data_array[t + 1, :]])
...
def get_reward(bs, actions, learnable_tree, tree_params, tree_optim):
    ...
    for bs_idx in range(batch_size):
        ...
        tree_optim = torch.optim.Adam(tree_params, lr=0.05)
        for current_iter in range(2000):
            cosine_lr(tree_optim, 5e-2, current_iter, 2000)
...
def train_controller(Controller, Controller_optim, trainable_tree, tree_params, hyperparams):
    ...
    for step in range(hyperparams['controller_max_step']):
        ...
        # actions = [torch.tensor([2], dtype=torch.int32), torch.tensor([1], dtype=torch.int32), torch.tensor([2], dtype=torch.int32), torch.tensor([0], dtype=torch.int32), torch.tensor([2], dtype=torch.int32)]
        ...
        error = rewards
        rewards = torch.exp(-torch.log10(rewards)/2)/math.exp(8/2)#1 / (1 + (rewards)**(0.05))
    ...
    for candidate_ in candidates.candidates:
        ...
        for current_iter in range(finetune):
            ...
            leaves_cnt = 0
            count = 0
            suffix = 'Finetune-- Iter {current_iter} Error {error:.10f} Formula {formula}'.format(current_iter=current_iter, error=error, formula=formula)
...
if __name__ == '__main__':
    ...
    total_params_controller = sum(p.numel() for p in controller.parameters() if p.requires_grad)
    total_params_trainable_tree = sum(p.numel() for p in trainable_tree.parameters() if p.requires_grad)

    print(f"Total Trainable Parameters in Controller: {total_params_controller}")
    print(f"Total Trainable Parameters in Learnable Computation Tree: {total_params_trainable_tree}")

    # Total overall trainable parameters
    total_params = total_params_controller + total_params_trainable_tree
